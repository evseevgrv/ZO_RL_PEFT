...
class OurArguments(TrainingArguments):
    ...
    log_dir: str = None  # base directory for local run artifacts; if None, will use "logs"
...
class Framework:
    ...
    def train(self, train_samples, dev_samples, eval_samples):
        ...
        self.last_global_step = trainer.state.global_step
...
def main():
    ...
    if not args.tag:
        ...
        if args.trainer == "zo_rl_jaguar" or args.trainer == "zo_rl_sgd" or args.trainer == "zo_rl_adamm":
    ...
    if args.log_to_file:
        ...
        args.log_run_dir = init_local_run_logger(
            log_dir=log_dir,
            run_tag=args.tag,
    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if args.log_to_file:
                log_local_metrics(
                    metrics,
                    step=getattr(framework, "last_global_step", None),
                    phase="eval",
                )
        ...
        if args.use_wandb:
            wandb.log(metrics)
        if args.log_to_file:
            log_local_metrics(metrics, phase="icl_eval")
        # for key, value in metrics.items():
        #     clearml_task.get_logger().report_scalar(
        #         title=key,
        #         series=key,
