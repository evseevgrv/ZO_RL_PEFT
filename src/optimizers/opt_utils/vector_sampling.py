...
class VectorSampler:
    ...
    def _lp_uniform_sphere(self, param_shape, p=2.0, device=None, generator=None):
        ...
        if p == 'inf':
            ...
            return torch.randint(0, 2, param_shape, device=device, generator=gen) * 2 - 1

        if p == 2.0:
            # For L2 norm, we can use the standard Gaussian method
            x = torch.randn(param_shape, device=device, generator=gen)
            norm = torch.norm(x, p=2, dim=-1, keepdim=True)
            return x / norm

        elif p == 1.0:
            # For L1 norm, we can use the Dirichlet distribution
            exp_samples = torch.empty(param_shape, device=device).exponential_(generator=gen)
            l1_norm = torch.sum(exp_samples, dim=-1, keepdim=True)
            samples = exp_samples / l1_norm
            signs = torch.randint(0, 2, param_shape, device=device, generator=gen) * 2 - 1
